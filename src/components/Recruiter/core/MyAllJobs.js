    ...
    <React.Fragment>
        ...
        <div className="flex-container mt-3">
            <div className="flex-element">
            {
                ...
                (
                <React.Fragment>
                    <Button variant="info" href={`/recruiter/jobs/applied-candidate-list/${props.jobDetail._id}`}>Applied Candidates ({props.jobDetail.appliedUser.length})</Button> &nbsp;
                    <Button variant="success" href={`/recruiter/jobs/shortlisted-candidate-list/${props.jobDetail._id}`}>Shortlisted Candidates ({props.jobDetail.shortListedUser.length})</Button> &nbsp;
                    <Button variant="danger" href={`/recruiter/jobs/rejected-candidate-list/${props.jobDetail._id}`}>Rejected Candidates ({props.jobDetail.rejectedUser.length})</Button> &nbsp;
                </React.Fragment>
                )
                :
                null                              
            }
            {
                ...
                <Button variant="success" href={`/recruiter/jobs/approval-request/${props.jobDetail._id}`}>Approval Request</Button>
                :
                null                              
            }
            </div>
...
export default function MyAllJobs() {
    ...
    const loadJobs = () => {
        ...
        axios({
            method: 'POST',
            url: `${process.env.REACT_APP_API}/recruiter/jobs/page/${currentPage}`,
            headers: {
    ...
    const loadPages = () => {
        ...
        axios({
            method: 'POST',
            url: `${process.env.REACT_APP_API}/recruiter/jobs/pagecount`,
            headers: {
                Authorization: `Bearer ${token}`
            },
            data: {RecruiterId}
