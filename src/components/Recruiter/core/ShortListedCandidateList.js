...
export default function ShortListedCandidateList() {
    ...
    const loadCandidates = () => {
        axios({
            method: 'POST',            
            url: `${process.env.REACT_APP_API}/recruiter/jobs/shortlisted-candidate-list/${jobId}/${currentPage}`,
            headers: {
    ...
    const loadPages = () => {
        axios({
            method: 'POST',
            url: `${process.env.REACT_APP_API}/recruiter/jobs/shortlisted-candidate-list/pagecount/${jobId}`,
            headers: {
                Authorization: `Bearer ${token}`
            },
            data: {RecruiterId}
    ...
    const handleReject = CandidateId => event => {
        event.preventDefault();
        axios({
            method: 'PUT',
            url: `${process.env.REACT_APP_API}/recruiter/jobs/reject/${jobId}`,
            headers: {
                Authorization: `Bearer ${token}`
            },
            ...
            .then(response => {
                console.log('Successfully rejected candidate', response);                
                loadCandidates();
                toast.success('Successfully rejected candidate');
            })
            .catch(error => {
        ...
        <React.Fragment>   
            ...
            <div className="row">
                <div className="col-md-12 mx-auto">
                    <h1 className="text-center mb-5">Short Listed Candidates</h1>
