...
const handleApply = event => {
    ...
    if(isAuth() && isAuth().role === 'user') {
        if(isAuth().ProfileCompleted) {
            const JobId = props.jobDetail._id;
            const UserId = isAuth()._id;
            const RecruiterId = props.jobDetail.recruiterId;
            axios({
                method: 'POST',
                url: `${process.env.REACT_APP_API}/users/jobapply/${JobId}`,
                headers: {
                    Authorization: `Bearer ${token}`
                },
                data: { JobId, UserId, RecruiterId }
            })
                .then(response => {            
                    if(response.data.error !== '') {
                        ...
                        console.log('Successfully applied for job', response);  
                        toast.dismiss(); 
            ...
            if(!isAuth().PersonalDetailCompleted)  {
                history.push('/users/personaldetails');
            }
            else if(!isAuth().EducationDetailCompleted) {
                history.push('/users/educationdetails');
            }      
        }
    }
    else {
        console.log('Signin To Apply');
...
export default function JobPostings() {
    ...
    const loadJobs = () => {
        axios({
            method: 'GET',
            url: `${process.env.REACT_APP_API}/users/jobs/page/${currentPage}`,
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
    ...
    const loadPages = () => {
        axios({
            method: 'GET',
            url: `${process.env.REACT_APP_API}/users/jobs/pagecount`,
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
    ...
    return(
        <section className="job-listing-section">
            ...
            {/* {console.log("jobs in react" ,jobs)} */}
            {
                jobs    &&
                jobs.map(job => (
                <Accordion key={job._id + 'accordian'} className="job-listing-card">
